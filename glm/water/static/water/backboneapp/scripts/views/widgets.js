...
app.TwoColumnsTableView = Backbone.View.extend({
  template: _.template($('#twocolumns-table-template').html()),
  ...
  render: function() {
    ...
    var that = this;
    this.model.data.forEach(function(item) {
      var itemView = new app.TwoColumnsTableItemView({
        el: $('#' + that.model.tagId),
        model: item
      }); 
      itemView.render();
...
app.TwoColumnsTableItemView = Backbone.View.extend({
  template: _.template($('#twocolumns-table-item-template').html()),
...
app.FourIndicatorsView = Backbone.View.extend({
